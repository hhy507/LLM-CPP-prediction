...
def plot_bit_counts(count_0, count_1):
    ...
    # 创建自定义颜色映射
    blue_cmap = create_cmap(0xFFA500)  # 橙色系 (原蓝色)
    pink_cmap = create_cmap(0x5F9EA0)  # 深青色系 (原粉色)
    ...
    # 添加图例
    ax.bar(0, 0, color=blue_cmap(1), label='1(orange)')
    ax.bar(0, 0, color=pink_cmap(1), label='0(green)')
    ax.legend()

    plt.tight_layout()
    plt.savefig('gpt_0.png', dpi=1000, bbox_inches='tight')
    plt.show()
